...
const BillModal = ({ editing, onClose, defaultUserType = "Farmer" }) => {
  ...
  const [form, setForm] = useState({
    ...
    billDate: editing?.billDate ? editing.billDate.split("T")[0] : new Date().toISOString().slice(0,10),
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        const url = userType === "Farmer" ? "/api/v1/farmer/feed" : "/api/v1/merchant/list"; // implement merchant route
    ...
    <dialog open className="modal">
      <div className="modal-box">
        ...
        <div>
          ...
          <select value={form.userRef} onChange={e=>setForm({...form,userRef:e.target.value})}>
            <option value="">Select</option>
            {userList.map(u => <option key={u._id} value={u._id}>{u.name} — {u.village}</option>)}
          </select>
