...
const Bill = () => {
  ...
  const netTotal = Number(
    totalAmount - (commission + pattiCharges + advancePaid + externalVegCost),
  )
  ...
  const BillContent = () => (
    <div className=" w-full border-2 p-4  bg-white text-sm">
      ...
      <div className="flex justify-between border-b pb-2 mb-2">
        <div>
          <p>
            {generalInfo.userType} नाव: <b>{generalInfo.userName || '____'}</b>
          </p>
          <p>
            फोन नंबर: <b>{generalInfo.userMobile || '____'}</b>
          </p>
          <p>
            मालक नाव: <b>योगेश गोटेफोडे</b>
          </p>
        </div>
        <div>
          <p>
      ...
      <div className="text-right w-full mt-2 space-y-0.5">
        <div className="">
          ...
          <p className={userType === 'farmer' ? 'pr-[8%]' : 'pr-[11%]'}>
            नगदी (-): ₹{advancePaid.toFixed(0)}
          </p>
          <p className={userType === 'farmer' ? 'pr-[8%]' : 'pr-[11%]'}>
            इतर माल (-): ₹{externalVegCost.toFixed(0)}
          </p>
          <hr />
          <p
            className={
              userType === 'farmer'
                ? 'pr-[8%]  text-red-800 text-md mt-1'
                : 'pr-[11%] font-bold  text-red-800 text-md mt-1'
            }
          >
