...
const BillView = () => {
  ...
  const generatePDF = () => {
    ...
    doc.setFont('NotoDev', 'bold')
    doc.setFontSize(12)
    doc.text(`अंतिम रक्कम: ₹${bill.netTotal}`, rightX + 30, finalY + 30, {align: 'right'})
          ...
          <div className="border border-[#212324] p-8">
            ...
            <div className="flex justify-between mb-6 text-sm text-[#12202E]">
              ...
              <div className="space-y-1">
                <p>
                  merchant नाव: <span className="font-semibold">{bill.userName}</span>
                </p>
                <p>
                  फोन नंबर: <span className="font-semibold">{bill.userMobile}</span>
                </p>
                <p>
                  मालक नाव: <span className="font-semibold">{bill.brokerId}</span>
                </p>
              </div>
              ...
              <tbody>
                {bill.items?.map((item, index) => (
                  <tr key={item._id} className="text-sm">
                    <td className="border border-[#E6E9EA] px-3 py-2 text-center">
                      {index + 1}
                    </td>
                    <td className="border border-[#E6E9EA] px-3 py-2">
                      {item.productName}
                    </td>
                    <td className="border border-[#E6E9EA] px-3 py-2 text-center">
                      {item.weight}
