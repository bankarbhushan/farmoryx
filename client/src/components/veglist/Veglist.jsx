...
const Veglist = () => {
  ...
  const handlesearch = (e) => {
    ...
    const filtered = vegs.filter((f) => {
      return (
        f.hinglishName.toLowerCase().includes(value) ||
        f.englishName.toLowerCase().includes(value)
      )
    })
  ...
  const handleAddVeg = async () => {
    try {
      const res = await axios.post('http://localhost:3000/api/v1/veg/add', newVeg)
      ...
      <div className="flex justify-between items-center mb-5">
        <div>
          ...
          <p className="text-sm text-[#94A3B8] font-light mt-1">
            Manage all vegetables in the system.
          </p>
        </div>
              ...
              {filteredVegs.map((veg, index) => (
                <tr key={veg._id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 flex justify-center gap-3">
                    ...
                    <button
                      onClick={() =>
                        document.getElementById(`delete_modal_${veg._id}`).showModal()
                      }
