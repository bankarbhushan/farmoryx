...
const FarmerList = () => {
  ...
  const handleAddFarmer = () => {
    if (newFarmer.name && newFarmer.village && newFarmer.mobile) {
      ...
      {isLoading ? (
        ...
        <Wrapper>
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-xl font-bold text-gray-800">Farmer List</h1>
            <button
              onClick={openModal}
          ...
          {/* Add FarmerModal Form (visible when showForm = true) */}
          <FarmerModal
          showForm={showForm}
          newFarmer={newFarmer}
                ...
                {farmers.map((farmer, index) => (
                  <tr className="hover:bg-gray-50" key={index}>
                    ...
                    <td className="px-4 py-2 flex justify-center gap-2">
                      <button className="px-3 py-1 bg-yellow-700 text-white text-sm rounded hover:bg-yellow-600 transition">
                        Update
                      </button>
