...
const createBill = asyncHandler(async (req, res) => {
  ...
  // Extract calculation
  const {totalAmount, commissionAmount, netTotal} = calculation
...
const allbills = asyncHandler(async (req, res) => {
  try {
    ...
    return res.status(200).json(new ApiResponse(200, bills, 'Bill fetched successfully.'))
  } catch (error) {
    //  HANDLE MONGOOSE VALIDATION ERRORS
    if (error.name === 'ValidationError') {
      const validationMessage = Object.values(error.errors)
...
const deleteBill = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, deleteBill, 'Bill Deleted Successfully.'))
  } catch (error) {
    if (error.name === 'ValidationError') {
      const validationMessage = Object.values(error.errors)
        .map((err) => err.message)
...
const SingleBill = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, existingBill, 'Sigle bill data fetach Successfully.'))
  } catch (error) {
    if (error.name === 'ValidationError') {
      const validationMessage = Object.values(error.errors)
        .map((err) => err.message)
